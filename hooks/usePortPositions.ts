...
export function usePortPositions() {
  ...
  const updatePortPosition = useCallback((nodeId: string, portId: string, x: number, y: number, position: 'top' | 'right' | 'bottom' | 'left') => {
  ...
  const getPortPosition = useCallback((nodeId: string, portId: string): PortPosition | undefined => {
