...
const embedding_providers_1 = require("./embedding-providers");
class EmbeddingService {
    ...
    createVendor(config) {
        ...
        return (0, embedding_providers_1.createEmbeddingVendor)(vendorConfig);
    }
    getDefaultModel(vendor) {
        switch (vendor) {
            case 'openai':
    ...
    async generateEmbeddings(template) {
        try {
            ...
            if (this.config.vendor !== 'mock') {
                console.warn('Falling back to mock embeddings due to vendor error. Search results may be less accurate.');
    ...
    async generateQueryEmbedding(query) {
        try {
            ...
            if (this.config.vendor !== 'mock') {
                console.warn('Falling back to mock embedding for query due to vendor error. Search results may be less accurate.');
    ...
    static getConfigFromEnvironment() {
        const vendor = (process.env.EMBEDDING_VENDOR || 'mock');
    ...
    generateCombinedText(template) {
        ...
        const inputPorts = (template.ports || [])
            ...
            .map(p => `input ${p.label} ${p.schema ? `accepts ${typeof p.schema === 'string' ? p.schema : 'data'}` : ''}`)
            .join(' ');
        const outputPorts = (template.ports || [])
            .filter(p => p.type === 'output')
            .map(p => `output ${p.label} ${p.schema ? `returns ${typeof p.schema === 'string' ? p.schema : 'data'}` : ''}`)
            .join(' ');
        if (inputPorts)
            parts.push(inputPorts);
        if (outputPorts)
            parts.push(outputPorts);
        ...
        const properties = Object.entries(template.properties)
            ...
            if (prop.description)
                propParts.push(prop.description);
            if (prop.required)
            ...
            if (prop.placeholder)
                propParts.push(`hint ${prop.placeholder}`);
        ...
        if (template.ports && Array.isArray(template.ports)) {
            template.ports.forEach(port => {
                if (port.type === 'input') {
                    ...
                    if (port.schema) {
                        capabilities.push(`input type ${typeof port.schema === 'string' ? port.schema : 'structured data'}`);
                    }
                }
                else {
                    capabilities.push(`produces ${port.label}`);
                    if (port.schema) {
                        capabilities.push(`output type ${typeof port.schema === 'string' ? port.schema : 'structured data'}`);
                    }
                }
            });
        }
    ...
    generateUseCaseText(template) {
        ...
        if (titleLower.includes('file'))
            useCases.push('file handling');
        if (titleLower.includes('transform'))
            useCases.push('data transformation');
        if (titleLower.includes('filter'))
            useCases.push('data filtering');
        if (titleLower.includes('aggregate'))
            useCases.push('data aggregation');
