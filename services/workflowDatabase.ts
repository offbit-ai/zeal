...
export class WorkflowDatabase {
  ...
  static async updateSnapshot(
    ...
    const existingResult = await db.query('SELECT * FROM workflow_snapshots WHERE id = $1', [
      snapshotId,
    ])
    ...
    const updatedResult = await db.query('SELECT * FROM workflow_snapshots WHERE id = $1', [
      snapshotId,
    ])
