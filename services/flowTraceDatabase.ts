...
export class FlowTraceDatabase {
  ...
  static async addTrace(
    ...
    const isSubgraphNode =
      data.source.nodeType === 'subgraph' || data.target.nodeType === 'subgraph'
  ...
  static async getSession(sessionId: string): Promise<FlowTraceSession | null> {
    ...
    // Get session details
    const sessionResult = await db.query('SELECT * FROM flow_trace_sessions WHERE id = $1', [
      sessionId,
    ])
  ...
  static async deleteOldSessions(olderThan: Date): Promise<number> {
    const db = await getDatabase()
    const result = await db.query('DELETE FROM flow_trace_sessions WHERE "startTime" < $1', [
