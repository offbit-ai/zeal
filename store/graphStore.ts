...
export const useGraphStore = create<GraphStore>()(
  subscribeWithSelector((set, get) => ({
    ...
    setGraphDirty: (graphId: string, isDirty: boolean) => {
      set((state) => ({
        graphs: state.graphs.map(g => 
          g.id === graphId ? { ...g, isDirty } : g
        )
      }))
    },

    updateCanvasState: (graphId: string, canvasState: { offset: { x: number; y: number }; zoom: number }) => {
      set((state) => ({
        graphs: state.graphs.map(g => 
          g.id === graphId ? { ...g, canvasState } : g
        )
      }))
    },
    ...
    loadGraphs: (graphs: GraphInfo[], currentGraphId?: string) => {
      ...
      const validGraphs = graphs.length > 0 ? graphs : [{
        id: 'main',
        name: 'Main',
        namespace: 'main',
        isMain: true,
