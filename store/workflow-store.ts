  ...
  devtools(
    (set, get) => {
      ...
      return {
        ...
        updateNodeMetadata: (nodeId: string, metadata: NodeMetadata, saveSnapshot: boolean = false) => {
          const { doc, currentGraphId } = get()
          if (!doc) return
        ...
        removeNode: (nodeId: string) => {
          ...
          doc.transact(() => {
            ...
            groupsMap.forEach((yGroup: any, groupId: string) => {
              const nodeIds = yGroup.get('nodeIds') as string[]
              if (nodeIds.includes(nodeId)) {
                yGroup.set('nodeIds', nodeIds.filter(id => id !== nodeId))
              }
            })
          })
        ...
        updateGroup: (groupId: string, updates: Partial<NodeGroup>) => {
          ...
          doc.transact(() => {
            ...
            // Get current data
            const currentGroup = storedGroup instanceof Y.Map ? {
        ...
        updateGroupBounds: (groupId: string, bounds: Partial<{ x: number; y: number; width: number; height: number }>) => {
          ...
          if (deltaX !== 0 || deltaY !== 0) {
            const memberNodes = nodes.filter(node => group.nodeIds.includes((node as any).id || node.metadata.id))
          ...
          const provider = get().provider
          if (provider && provider.socket && provider.socket.connected) {
            // Force immediate sync by triggering awareness update
            const awareness = provider.getAwareness()
            const currentState = awareness.getLocalState() || {}
            awareness.setLocalState({
              ...currentState,
        ...
        recalculateGroupBounds: (groupId: string) => {
          ...
          const memberNodes = nodes.filter(n => group.nodeIds.includes((n as any).id || n.metadata.id))
        ...
        addNodeToGroup: (nodeId: string, groupId: string) => {
          ...
          (doc as any).transact(() => {
            ...
            (get() as any).recalculateGroupBounds(groupId)
          ...
          return graphs.map(graph => {
            ...
            groupsMap.forEach((groupValue: any, key: string) => {
              ...
              if (groupValue instanceof Y.Map) {
                ...
                console.warn('[WorkflowStore] getAllGraphsData: Unknown group format:', typeof groupValue)
                return
              }
...
function setupObservers(doc: Y.Doc, set: any, get: any) {
  ...
  loadGraphData(doc, currentGraphId, set)
}

function loadGraphData(doc: Y.Doc, graphId: string, set: any) {
  if (!doc) {
  ...
  const updatePresence = () => {
    ...
    awareness.getStates().forEach((state, clientId) => {
      if (state.user || state.userId) {
        states.set(clientId, {
          userId: state.user?.userId || state.userId || `client-${clientId}`,
          userName: state.user?.userName || state.userName || 'Anonymous',
          userColor: state.user?.userColor || state.userColor || '#3b82f6',
          cursor: state.cursor,
