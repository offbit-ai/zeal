...
async fn register_example_templates(client: &ZealClient) -> Result<()> {
    ...
    let templates = vec![
        NodeTemplate {
            ...
            properties: Some({
                ...
                props.insert("batch_size".to_string(), PropertyDefinition {
        ...
        NodeTemplate {
            ...
            properties: Some({
                let mut props = std::collections::HashMap::new();
                props.insert("schema".to_string(), PropertyDefinition {
...
async fn setup_event_subscription(client: &ZealClient, workflow_id: &str) -> Result<()> {
...
async fn handle_workflow_events(workflow_id: &str) -> Result<()> {
    ...
    let node_executing_event = create_node_executing_event(
        workflow_id,
        "data-processor-1",
    ...
    println!("     🔄 Node executing: {} in workflow {}", 
        node_executing_event.node_id, 
        node_executing_event.base.workflow_id
    );
    ...
    let node_completed_event = create_node_completed_event(
        workflow_id,
        "data-processor-1",
        vec!["connection-2".to_string()],
        Some(crate::events::NodeCompletedOptions {
            duration: Some(95),
    ...
    println!("     ✅ Node completed: {} ({}ms, {} bytes)", 
...
async fn simulate_workflow_execution(client: &ZealClient, workflow_id: &str) -> Result<()> {
    println!("   Starting execution simulation for workflow: {}", workflow_id);

    // Create execution started event
    let execution_started = create_execution_started_event(
        workflow_id,
        &format!("session-{}", uuid::Uuid::new_v4()),
        "Rust SDK Example Workflow",
        Some(crate::events::ExecutionStartedOptions {
            trigger: Some(crate::events::ExecutionTrigger {
    ...
    println!("     🚀 Execution started: {}", execution_started.session_id);
    ...
    for i in 1..=3 {
        ...
        // Node executing
        let executing_event = create_node_executing_event(
            workflow_id,
            &node_id,
            if i == 1 { vec![] } else { vec![format!("conn-{}", i-1)] },
            Some("main".to_string()),
        );
        ...
        // Node completed
        let completed_event = create_node_completed_event(
            workflow_id,
            &node_id,
            vec![format!("conn-{}", i)],
            Some(crate::events::NodeCompletedOptions {
    ...
    let execution_completed = create_execution_completed_event(
        workflow_id,
        &execution_started.session_id,
        285, // Total duration
        3,   // Nodes executed
        Some(crate::events::ExecutionCompletedOptions {
            summary: Some(crate::events::ExecutionSummary {
                success_count: 3,
                error_count: 0,
                warning_count: 0,
            }),
    ...
    println!("     🎉 Execution completed: {} ({}ms, {} nodes)", 
