...
export function GraphDebugPanel() {
  ...
  return (
    <div style={{
      ...
      <div>Current Graph: <strong>{currentGraphId}</strong></div>
      <div>UI Nodes: <strong>{nodes.length}</strong></div>
      <hr style={{ margin: '8px 0' }} />
      <h4 style={{ margin: '8px 0' }}>CRDT Graph Data:</h4>
      {Object.entries(graphData).map(([graphId, data]) => (
        <div key={graphId} style={{ 
          marginBottom: 8,
          padding: 8,
          background: graphId === currentGraphId ? '#e3f2fd' : '#f5f5f5',
          borderRadius: 4
        }}>
          <div><strong>{data.name}</strong> ({graphId})</div>
          <div>Nodes in CRDT: {data.nodeCount}</div>
          {data.nodes.length > 0 && (
            <ul style={{ margin: '4px 0 0 20px', padding: 0 }}>
              {data.nodes.map((node: any) => (
                <li key={node.id}>{node.title} ({node.type})</li>
              ))}
            </ul>
          )}
        </div>
