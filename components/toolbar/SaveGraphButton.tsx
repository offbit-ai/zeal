...
export function SaveGraphButton({
  ...
  return (
    <div
      className={`fixed left-6 bottom-6 z-50 transition-all duration-200 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'}`}
    >
      <button
        onClick={onSave}
        disabled={isSaving}
      ...
      >
        <Save
          className={`w-4 h-4 ${isSaving ? 'animate-pulse' : 'group-hover:scale-110 transition-transform'}`}
        />
