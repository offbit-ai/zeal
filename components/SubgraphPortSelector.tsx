...
export function SubgraphPortSelector({ 
  ...
  const filteredNodes = useMemo(() => {
    ...
    return subgraphNodes.filter(node => 
    ...
    <ModalPortal isOpen={isOpen}>
      ...
      >
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div>
            <h2 className="text-xl font-semibold text-gray-900">Select Input Port from Subgraph</h2>
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className={`p-2 rounded ${
                          ...
                          <Icon name={node.metadata.icon || 'box'} className="w-4 h-4 text-gray-700" />
                        </div>
                ...
                <div className="p-4 border-b border-gray-200">
                  <div className="flex items-center gap-3">
                    <div className="p-2 bg-gray-100 rounded">
                      <Icon name={selectedNode.metadata.icon || 'box'} className="w-5 h-5 text-gray-700" />
                    </div>
                    <div>
                      <div className="font-medium text-gray-900">{selectedNode.metadata.title}</div>
                      <div className="text-sm text-gray-500">{selectedNode.metadata.subtitle}</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-6">
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {selectedNode.metadata.ports?.filter(p => p.type === 'input')
                        .map(port => (
                          <button
                            key={port.id}
                      ...
                      {selectedNode.metadata.ports?.filter(p => p.type === 'input').length === 0 && (
