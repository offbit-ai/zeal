...
import { SubgraphPortSelector } from '../node/SubgraphPortSelector'
...
export function SearchModal({
  ...
  const [activeTab, setActiveTab] = useState<'repository' | 'custom' | 'subgraphs'>(
  ...
  const saveCustomNode = (nodeTemplate: any, isEditing: boolean = false) => {
    ...
    if (isEditing && editingNode) {
      ...
      updatedNodes = customNodes.map(node => (node.id === editingNode.id ? customNode : node))
    } else {
      // Add new node
  ...
  const handleAddSubgraph = (graphId: string) => {
    ...
    if (addedNodeId) {
      ...
      if (canvasOffset && canvasZoom && viewportSize && onCanvasOffsetChange && onHighlightNode) {
        const targetOffset = calculatePanToCenter(position, viewportSize, canvasOffset, canvasZoom)
  ...
  const handleAddToCanvas = async (nodeItem: NodeRepositoryItem) => {
    ...
    try {
      instanceMetadata = (await updateDynamicNodeMetadata(
        ...
        propertyValues,
        nodeItem.template.propertyRules
      )) as any
    } catch (error) {
    ...
    >
                          ...
                          >
                            ...
                            <div className="flex items-center gap-4 text-xs text-gray-500 mb-4">
                              <span className="flex items-center gap-1">
                                ...
                                {node.template.type === 'script'
                                  ? node.template.variant?.includes('blue')
                                    ? 'Python'
                                    : 'JavaScript'
                                  : 'API'}
                              </span>
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
              <div className="p-4 border-b border-gray-200">
                ...
                <p className="text-sm text-gray-500 mt-1">
                  Add references to other graphs in your workflow
                </p>
              </div>
      ...
      <NodeCreatorModal
        ...
        editingNode={{
          ...
          propertyValues: editingNode?.template.properties
            ? Object.entries(editingNode?.template.properties as any)
            : {},
