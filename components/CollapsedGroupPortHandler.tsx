...
export function CollapsedGroupPortHandler({
  ...
  useEffect(() => {
    ...
    updateTimeoutRef.current = setTimeout(() => {
      ...
      groupNodes.forEach(node => {
        ...
        metadata.ports?.filter(p => p.type === 'input').forEach(port => {
          onPortPositionUpdate(
            nodeId, 
            port.id, 
            leftEdgeX,
        ...
        metadata.ports?.filter(p => p.type === 'output').forEach(port => {
          onPortPositionUpdate(
            nodeId, 
            port.id, 
            rightEdgeX,
  ...
  }, [groupId, groupPosition.x, groupPosition.y, groupSize.width, groupSize.height, groupNodeIds, nodes, onPortPositionUpdate])
