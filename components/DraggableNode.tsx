...
interface DraggableNodeProps {
  ...
  onNodeNearGroupBoundary?: (nodeId: string, groupId: string, action: 'resize-right' | 'resize-down', value?: number) => void  // Callback when node approaches group boundary
  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    const handleMouseMove = (e: globalThis.MouseEvent) => {
      ...
      if (isInGroup && groupContainer) {
        ...
        const groupRect = groupContainer.getBoundingClientRect()
        const contentDiv = groupContainer.querySelector('.absolute.left-0.right-0.bottom-0') as HTMLElement
      ...
      if (isInGroup && currentGroup) {
        ...
        if (onNodeNearGroupBoundary) {
          ...
          if (constrainedPosition.x + nodeWidth > (currentGroup.size?.width || 200) - resizeThreshold) {
          ...
          if (constrainedPosition.y + nodeHeight > (currentGroup.size?.height || 150) - resizeThreshold) {
  ...
  return (
    ...
    >
      <WorkflowNode 
        ...
        zoom={zoom}
        onPropertyChange={onPropertyChange ? (propertyName: string, value: any) => onPropertyChange(effectiveNodeId, propertyName, value) : undefined}
        onSettingsClick={metadata.type === 'script' && onClick ? () => onClick(effectiveNodeId) : undefined}
        onSizeChange={() => {
