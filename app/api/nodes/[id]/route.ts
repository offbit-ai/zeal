...
export const GET = withErrorHandling(
  async (req: NextRequest, context?: { params: { id: string } }) => {
    ...
    const nodeTemplate = nodeTemplatesStore.find(
      node => (node.id === id || node.type === id) && node.isActive
    )
...
export const PUT = withErrorHandling(
  async (req: NextRequest, context?: { params: { id: string } }) => {
    ...
    if (body.type && body.type !== existingNode.type) {
      const duplicateNode = nodeTemplatesStore.find(
        node => node.type === body.type && node.id !== id
      )
    ...
    const updatedNode = {
      ...
      requiredEnvVars:
        body.requiredEnvVars !== undefined ? body.requiredEnvVars : existingNode.requiredEnvVars,
