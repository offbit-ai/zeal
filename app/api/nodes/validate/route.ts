...
export const POST = withErrorHandling(async (req: NextRequest) => {
  ...
  const nodeTemplate = nodeTemplatesStore.find(template => 
    template.id === body.nodeTemplateId || template.type === body.nodeTemplateId
  )
  
  if (!nodeTemplate) {
    throw new ApiError('NODE_TEMPLATE_NOT_FOUND', `Node template '${body.nodeTemplateId}' not found`, 404)
  }
  ...
  Object.entries(nodeTemplate.properties).forEach(([propKey, propSchema]: [string, any]) => {
    ...
    if (propValue !== undefined && propValue !== null && propValue !== '') {
      ...
      if ((propSchema.type === 'code-editor' || propKey === 'script') && typeof propValue === 'string') {
        ...
        if (propValue.toLowerCase().includes('delete') || propValue.toLowerCase().includes('drop')) {
          warnings.push({
            field: propKey,
  ...
  const actuallyMissingVars = (nodeTemplate.requiredEnvVars || []).filter(varName => 
    !configuredEnvVars.includes(varName)
  )
