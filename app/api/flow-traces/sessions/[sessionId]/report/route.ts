  ...
  const report = {
    ...
    analysis: {
      ...
      successRate: traces.length > 0 
        ...
        : '0%',
      averageLatency: traces.length > 0
        ? (traces.reduce((sum, t) => sum + t.duration, 0) / traces.length).toFixed(2) + 'ms'
        : '0ms',
      totalDataTransferred: (traces.reduce((sum, t) => sum + t.data.size, 0) / 1024).toFixed(2) + 'KB',
      errorTraces: traces.filter(t => t.status === 'error').map(t => ({
...
-------------------
Slowest Traces:
${report.analysis.slowestTraces.map((trace, i) => `
${i + 1}. ${trace.flow}
   Duration: ${trace.duration}
